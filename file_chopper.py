...
FULL_DATA_PATH = "/Users/ericoliviera/Desktop/Data/smart-home-ksw/Time/Full_1"
CHOPPED_DIR = "/Users/ericoliviera/Desktop/Data/smart-home-ksw/Time/Full_1/chops2"
SAMPLE_RATE = 22050
DURATION = 0.75  # s. Duration of an utterance in seconds
WAIT = 0.5      # s. Silence buffer before threshold for a given utterance

# How much audio before the detected onset to include in each chop
PREPAD = 0.15    # s, tweak as needed (e.g. 0.1–0.3)

# Define valid audio extensions
AUDIO_EXTS = {"wav", "m4a", "flac", "mp3", "ogg", "opus", "aiff", "aif"}


def parse(thresh=0.1,
          wait=int(SAMPLE_RATE * WAIT),
          duration=int(SAMPLE_RATE * DURATION),
          plot_first=True):
    """
    This function assumes the dataset is uniform (i.e. already sanitized).
    ...
    for name in os.listdir(FULL_DATA_PATH):
        ...
        # From here on, only handle labeled speech files
        thresh_bad = True
        duration_bad = True
        markers = []
        audio_data = None
        ...
        while (thresh_bad or duration_bad):
            ...
            if plot_first:
                ...
                for k in range(0, len(markers), 2):
                    ...
                    s0 = max(0, beg) / sr
                    s1 = (beg + wait_samp) / sr
                    plt.axvspan(s0, s1, color="orange", alpha=0.25,
                                label=f"silence window ({(1000 * float(wait) / SAMPLE_RATE):.2f} ms)" if k == 0 else None)

                    # utterance window
                    u0 = beg / sr
                    u1 = end / sr
                    plt.axvspan(u0, u1, color="green", alpha=0.25,
                                label=f"utterance window ({(1000 * duration / sr):.2f} ms)" if k == 0 else None)
                ...
                try:
                    ...
                    thres_choice = int(input(
                        "\nIf the threshold is good, press 1. Else, press anything else to reset: "
                    ).strip() or '0')
                except Exception:
                ...
                try:
                    duration_choice = int(input(
                        "If the windowing is good, press 1. Else, press anything else to reset: "
                    ).strip() or '0')
                except Exception:
                    duration_choice = 0
                finally:
        ...
        if markers and audio_data is not None:
            ...
            try:
                ...
                wait_samp = int(wait)
                dur_samp = int(duration)
                prepad_samp = int(PREPAD * SAMPLE_RATE)  # NEW

                for k in range(0, len(markers), 2):
                    beg = int(markers[k])             # start of silence+utterance
                    trigger_idx = beg + wait_samp     # where |audio| first crossed thresh

                    # NEW: back up by prepad_samp to include quiet consonants ("t" in "time")
                    u_beg = max(0, trigger_idx - prepad_samp)
